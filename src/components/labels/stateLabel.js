...
// const StateLabelCheck = reStyle(BookingCheck)(()=> ({
//    fill: colors.primary,
//    maxHeight: 18,
// }))
...
export const StateLabel = props => {
    ...
    const labelToDisplay = (sessionState === SESSION_BOOKING_STATES.SELECT || sessionState === SESSION_BOOKING_STATES.READ_ONLY) 
      ? '' 
      : sessionState
    const stateLabelStyles = useStyle('stateLabel')
    ...
    return (
      <StateLabelView>
        ...
        <StateLabelText boldText={selected} disabledText={fullyBooked}>
          { labelToDisplay.toUpperCase() }
        </StateLabelText>
        { selected && 
            <BookingCheck 
              className='ef-button-text-icon' 
              style={stateLabelStyles.check} 
            /> 
            // TODO: Use StateLabelCheck component from retheme
            // <StateLabelCheck/>
        }
      </StateLabelView>
    )
